...
func TestAppStartOnBoot(t *testing.T) {
	app := NewApp(nil, nil, nil, nil)

	var bootState bool
	app.SetStartOnBootHandlers(
		func() (bool, error) {
	...
	val, err := app.GetStartOnBoot()
	if err != nil {
		t.Fatalf("GetStartOnBoot() error = %v", err)
	}
	if val != false {
		t.Errorf("GetStartOnBoot() = %v, want false", val)
	}

	if err := app.SetStartOnBoot(true); err != nil {
		t.Fatalf("SetStartOnBoot(true) error = %v", err)
	}
	if !bootState {
		t.Error("SetStartOnBoot(true) did not update bootState")
	}

	val, err = app.GetStartOnBoot()
	if err != nil {
		t.Fatalf("GetStartOnBoot() after set error = %v", err)
	}
	if val != true {
		t.Errorf("GetStartOnBoot() = %v, want true", val)
	}

	if err := app.SetStartOnBoot(false); err != nil {
		t.Fatalf("SetStartOnBoot(false) error = %v", err)
	}
	if bootState {
		t.Error("SetStartOnBoot(false) did not clear bootState")
	}
}
